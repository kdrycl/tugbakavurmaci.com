...
## GÖRSEL YÜKLENİYOR ##
  ...
  $targetPath = "../../tasks/";
  $targetFile = $targetPath . $_FILES['imageUpload']['name'];
    ...
    if ($shortCodeControl > 0) {
      ...
      if ($langAdd) {
        ...
        foreach ($categoriesGet as $key => $value) {
          ...
          if ($categoryLangControl <= 0) $categoryadd = $dbclass->ekle("categories", "categoryName,categorySlug,mainCategoryId,uniqueId,categoryDescription,categoryType,langCode,seoTitle,seoKeywords,seoDescription,recordStatus", array("", "", $value['mainCategoryId'], $value['uniqueId'], "", $value['categoryType'], $_POST['langShortCode'], "", "", "", 0));
        }
        ## YENİ DİL OLUŞTURULDUKTAN SONRA BU ZAMANA KADAR OLUŞTURULAN KATEGORİLER İÇİN YENİ EKLENEN DİLE GÖRE YENİ KAYITLARI OLUŞTURULUYOR ##
        ## YENİ DİL OLUŞTURULDUKTAN SONRA BU ZAMANA KADAR OLUŞTURULAN SAYFALAR İÇİN YENİ EKLENEN DİLE GÖRE YENİ KAYITLARI OLUŞTURULUYOR ##
        $pagesGet = $dbclass->cek("ASSOC_ALL", "pages", "DISTINCT uniqueId,pageName,pageSlug,pageContent,langCode,seoTitle,seoKeywords,seoDescription,recordStatus", "WHERE langCode=?", array("TR"));
        foreach ($pagesGet as $key => $value) {
          $pageLangControl = $dbclass->cek("KAYITSAY", "pages", "count(id)", "WHERE uniqueId=? AND langCode=?", array($value['uniqueId'], $_POST['langShortCode']));
          if ($pageLangControl <= 0) $pageAdd = $dbclass->ekle("pages", "pageName,pageSlug,pageContent,uniqueId,langCode,seoTitle,seoKeywords,seoDescription,recordStatus", array("", "","", $value['uniqueId'], $_POST['langShortCode'], "", "", "", 0));
        }
        ## YENİ DİL OLUŞTURULDUKTAN SONRA BU ZAMANA KADAR OLUŞTURULAN SAYFALAR İÇİN YENİ EKLENEN DİLE GÖRE YENİ KAYITLARI OLUŞTURULUYOR ##
         ## YENİ DİL OLUŞTURULDUKTAN SONRA BU ZAMANA KADAR OLUŞTURULAN DİL DEĞİŞKENLERİ İÇİN YENİ EKLENEN DİLE GÖRE YENİ KAYITLARI OLUŞTURULUYOR ##
        $siteConstGet = $dbclass->cek("ASSOC_ALL", "siteConstants", "DISTINCT constCode,pageCode,constValue,constSlug,slugUsage,lang", "WHERE lang=?", array("TR"));
        foreach ($siteConstGet as $key => $value) {
          $siteConstControl = $dbclass->cek("KAYITSAY", "siteConstants", "count(id)", "WHERE constCode=? AND lang=?", array($value['constCode'], $_POST['langShortCode']));
          if ($siteConstControl <= 0) $constAdd = $dbclass->ekle("siteConstants", "pageCode,constCode,constValue,constSlug,slugUsage,lang", array($value['pageCode'], $value['constCode'],$value['constValue'], $value['constSlug'],$value['slugUsage'], $_POST['langShortCode']));
        }
        ## YENİ DİL OLUŞTURULDUKTAN SONRA BU ZAMANA KADAR OLUŞTURULAN DİL DEĞİŞKENLERİ İÇİN YENİ EKLENEN DİLE GÖRE YENİ KAYITLARI OLUŞTURULUYOR ##
...
## SİTE DEĞİŞKENLERİ GÜNCELLENİYOR ##
  ...
  else echo json_encode(array("error","Kritik Hata!!!"));
}
## SİTE DEĞİŞKENLERİ GÜNCELLENİYOR ##
################################# DİL İŞLEMLERİ #################################
################################# KATEGORİ İŞLEMLERİ #################################
  ...
  if ($permissionControl == 1) { // kullanıcının ürün ekleme ve düzenleme yetkisi kontrol ediliyor
    ...
    if ($_POST['productProcess'] == "add") {
      ...
      if ($productAdd) {
        ...
        if($_POST['productCategory']) $categoryAdd = $dbclass->ekle("productsVsCategories", "productUniqueId,categoryUniqueId", array($productDetected['uniqueId'], $_POST['productCategory']));
...
## BLOG EKLEME VE GÜNCELLEME İŞLEMLERİ ##
  ...
  if ($permissionControl == 1) { // kullanıcının blog yazısı ekleme ve düzenleme yetkisi kontrol ediliyor
    ...
    if ($_POST['blogProcess'] == "add") {
      ...
      $blogAdd = $dbclass->ekle("blogs", "blogName,blogSlug,uniqueId,blogShortDescription,blogLongDescription,seoTitle,seoKeywords,seoDescription,recordStatus",
        array($_POST['blogName'], $blogLink, $uniqueId, $_POST['blogShortDescription'], $_POST['blogLongDescription'], $_POST['seoTitle'], $_POST['seoKeywords'], $_POST['seoDescription'], 1));
      if ($blogAdd) {
...
## BLOG SİLİNİYOR ##
################################# BLOG İŞLEMLERİ #################################
################################# FİRMA BİLGİLERİ DÜZENLENİYOR #################################
else if(isset($_POST['companyInfoDetail'])){
  $infoUpdate = $dbclass->guncelle(0, "companyInfo", "companyName,phone1,phone2,phone3,facebook,instagram,twitter,mail1,mail2,adress,mapCode", "WHERE id=?", 
  array($_POST['companyName'], $_POST['phone1'], $_POST['phone2'], $_POST['phone3'], $_POST['facebook'], $_POST['instagram'],$_POST['twitter'],$_POST['mail1'],$_POST['mail2'],$_POST['adress'],$_POST['mapCode'],1));
  if($infoUpdate) echo json_encode(array("success","Firma Bilgileri Başarıyla Güncellendi."));
  else echo json_encode(array("error","Firma Bilgileri Güncellenirken Bir Hata İle Karşılaşıldı."));

}
################################# FİRMA BİLGİLERİ DÜZENLENİYOR #################################
################################# SAYFA İŞLEMLERİ #################################
## SAYFALAR LİSTELENİYOR ##
else if (isset($_SERVER['HTTP_PAGELIST'])) {
  $pagesGet = $dbclass->cek("ASSOC_ALL", "pages", "*", "WHERE langCode=?", array("TR"));
  $data['data'] = array();
  $actions = array();
  foreach ($pagesGet as $key => $value) {
    $categoryControl = $dbclass->cek("KAYITSAY", "categories", "count(categories.id)", "INNER JOIN  pagesVsCategories ON pagesVsCategories.categoryId = categories.uniqueId WHERE pagesVsCategories.pageId=? AND categories.langCode=?", array($value['uniqueId'], "TR"));
    if ($categoryControl > 0) {
      $categoryDetected = $dbclass->cek("ASSOC", "categories", "categories.categoryName", "INNER JOIN  pagesVsCategories ON pagesVsCategories.categoryId = categories.uniqueId WHERE pagesVsCategories.pageId=? AND categories.langCode=?", array($value['uniqueId'], "TR"));
      $pageCategory = $categoryDetected['categoryName'];
    } else $pageCategory = "Kategori Bulunamadı";
    $langRows = "";
    $activeLangs = $dbclass->cek("ASSOC_ALL", "languages", "*", "WHERE status=?", array(1));
    foreach ($activeLangs as $keys => $values) {
      $langRows .= '
        <li class="navi-item">
          <a href="sayfa-detay-' . $value['uniqueId'] . '-' . $values['langShortCode'] . '" target="_blank" class="navi-link">
            <span class="navi-icon"><i class="la la-flag"></i></span>
            <span class="navi-text">' . $values['langName'] . '</span>
          </a>
        </li>
    ...
    $actions[0] = $langRows;
    $actions[1] = $value['id'];
    array_push($data['data'], array("pageNumber" => $key + 1, "name" => $value['pageName'], "category" => $pageCategory, "pageStatus" => $value['recordStatus'], "actions" => $actions));
  }
  echo json_encode($data['data'], JSON_UNESCAPED_UNICODE);

}
## SAYFALAR LİSTELENİYOR ##
## SAYFA EKLEME VE GÜNCELLEME İŞLEMLERİ ##
else if (isset($_POST['pageProcess'])) {
  $permissionControl = $functions->permissionControl($dbclass, $_SESSION['userid']['id'], 18);
  if ($permissionControl == 1) { // kullanıcının sayfa ekleme ve düzenleme yetkisi kontrol ediliyor
    ## YENİ SAYFA EKLENİYOR ##
    if ($_POST['pageProcess'] == "add") {
      $uniqueId = $dbclass->uniqueId("pages"); // sayfalar tablosunda benzeri olmayan uniqueId üretiliyor
      $pageLink = $dbclass->seo($_POST['pageName']); // gelen sayfa adına göre seo uyumlu slug oluşturuluyor
      $pageLink = $pageLink . "-" . $uniqueId;
      $pageImage= null;
      if(isset($_SESSION['imageDetails'])) $pageImage = substr($_SESSION['imageDetails'][0]['imageLink'],6);
      $now = new DateTime();
      $timestring = $now->format('Y-m-d h:i:s');
      $pageAdd = $dbclass->ekle("pages", "pageName,pageSlug,uniqueId,pageContent,seoTitle,seoKeywords,seoDescription,recordStatus,langCode,pageCode",
        array($_POST['pageName'], $pageLink, $uniqueId, $_POST['pageContent'], $_POST['seoTitle'], $_POST['seoKeywords'], $_POST['seoDescription'], 1,$_POST['pageLang'],0));
      if ($pageAdd) {
        $pageDetected = $dbclass->cek("ASSOC", "pages", "id,uniqueId,pageName", "ORDER BY id DESC", array());
        if (isset($_SESSION['imageDetails'])) { // sayfa eklenirken yüklenen tanıtım görseli var ise görsel veritabanına kaydediliyor
          foreach ($_SESSION['imageDetails'] as $key => $value) {
            $imageUpload = $functions->imageUpload($dbclass, 7, 0, $pageDetected['id'], $pageDetected['pageName']);
          }
        }
        if (isset($_POST['pageCategory'])) {
          $categoryAdd = $dbclass->ekle("pagesVsCategories", "pageId,categoryId", array($pageDetected['uniqueId'], $_POST['pageCategory']));
        }
        echo json_encode(array("success", "Kayıt Başarı İle Eklendi."));
      } else echo json_encode(array("error", "Kayıt Eklenirken Bir Hata İle Karşılaşıldı."));
    ...
    ## YENİ SAYFA EKLENİYOR ##
    ## SAYFA GÜNCELLENİYOR ##
    if ($_POST['pageProcess'] == "update") {
      $pageControl = $dbclass->cek("KAYITSAY", "pages", "count(id)", "WHERE id=?", array($_POST['id']));
      if ($pageControl > 0) {
        $pageDetected=$dbclass->cek("ASSOC","pages","*","WHERE id=?",array($_POST['id']));
        $pageLink = $dbclass->seo($_POST['pageName']); // gelen sayfa adına göre seo uyumlu slug oluşturuluyor
        $pageLink = $pageLink . "-" . $pageDetected['uniqueId'];
        $pageUpdate = $dbclass->guncelle(0, "pages", "pageName,pageSlug,pageContent,seoTitle,seoKeywords,seoDescription,recordStatus,langCode,pageCode", "WHERE id=?", 
        array($_POST['pageName'], $pageLink, $_POST['pageContent'], $_POST['seoTitle'], $_POST['seoKeywords'], $_POST['seoDescription'], 1,$pageDetected['langCode'],0, $_POST['id']));
        if ($pageUpdate) {
          if (isset($_SESSION['imageDetails'])) { // ürün eklenirken yüklenen tanıtım görseli var ise görsel veritabanına kaydediliyor
            $allImagesDelete = $dbclass->sil("images", "WHERE imageType=? AND sourceId=?", array(7, $pageDetected['uniqueId']));
            foreach ($_SESSION['imageDetails'] as $key => $value) {
              $imageUpload = $functions->imageUpload($dbclass, 7, 0, $pageDetected['uniqueId'], $pageDetected['pageName']);
            }
          }
          if (isset($_POST['pageCategory'])) {
            $allCategoriesDelete = $dbclass->sil("pagesVsCategories", "WHERE pageId=?", array($pageDetected['uniqueId']));
            $categoryAdd = $dbclass->ekle("pagesVsCategories", "pageId,categoryId", array($pageDetected['uniqueId'], $_POST['pageCategory']));
          }
          echo json_encode(array("success", "Kayıt Başarı İle Güncellendi."));
        } else echo json_encode(array("error", "Kayıt Güncellenirken Bir Hata İle Karşılaşıldı."));
    ...
    echo json_encode(array("error", "Sayfa Ekleme ve Düzenleme Yetkiniz Bulunmamaktadır."));
  }
}
## SAYFA EKLEME VE GÜNCELLEME İŞLEMLERİ ##
## SAYFA SİLİNİYOR ##
else if (isset($_POST['deletePageId'])) {
  $permissionControl = $functions->permissionControl($dbclass, $_SESSION['userid']['id'], 19);
  if ($permissionControl == 1) { // kullanıcının, yönetim panelinde ürün silme izni kontrol ediliyor
    $pageDelete = $dbclass->sil("pages", "WHERE id=?", array($_POST['deletePageId']));
    if ($pageDelete) {
      echo json_encode(array("success", "Silme İşlemi Başarıyla Gerçekleştirildi"));
    } else {
      echo json_encode(array("error", "Silme İşlmesi Esnasında Bir Hata Oluştu"));
    ...
    echo json_encode(array("error", "Sayfa Silme Yetkiniz Bulunmamaktadır."));
  }
}
## SAYFA SİLİNİYOR ##
################################# SAYFA İŞLEMLERİ #################################
################################# SLİDER İŞLEMLERİ #################################
  ...
  $imageDetected = $dbclass->cek("ASSOC_ALL", "images", "*", "WHERE imageType=? ORDER BY deviceType ASC", array(0));
  $data['data'] = array();
  foreach ($imageDetected as $key => $value) {
    $html='<img src="'.$value['imageLink'].'" style="max-height:50px;">';
    array_push($data['data'], array("sliderNumber" => $key + 1, "image" => $value['imageLink'], "deviceType"=>$value['deviceType'], "actions" => $value['id']));
  }
