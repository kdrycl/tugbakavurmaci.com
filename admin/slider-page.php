...
if ($sessionControl>0) { // kullanıcı giriş yapmış mı kontrol ediliyor giriş yapmışsa sayfaya yönlendiriliyor
  if (isset($_GET['id'])) {
    $productDetected=$dbclass->cek("KAYITSAY","images","COUNT(id)","WHERE id=?",array($_GET['id']));
    if ($productDetected>0) {
      $productSelect=$dbclass->cek("ASSOC","images","*","WHERE id=?",array($_GET['id'])); // düzenlenen kategorinin bilgileri getiriliyor
      $pageTitle='"'.$productSelect['imageAlt'].'"'." İsimli Galeri Bilgileri Düzenleniyor";
      $productAction="sliderUpdateInput";
      $productId=$_GET['id'];
      if ($productSelect['imageType']==1) {
        $status="slider";
        $fileSize=10;
      }
      else if ($productSelect['imageType']==2) {
        $status="banner";
        $fileSize=1;
      }
    }
