...
<main>
        ...
        <section>
            <div class="service-details-area pt-120 ">
                <div class="container">
                    <div class="row">
                        <div class="col-12">
        ...
        <?php 
            $countGallery = $dbclass->cek("KAYITSAY","servicesvsvideos","count(id)","where servicesId=?",array($detectedServices['id']));
            if($countGallery>0){ 
                $getGallery = $dbclass->cek("ASSOC_ALL","servicesvsvideos","*","where servicesId=? ORDER BY sort ASC",array($detectedServices['id']));
            ?>
            <section class="blog-area pt-20 pb-80">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-12">
                            <h2 class="text-center mt-3 mb-3 " style="font-weight: bolder;">Video Galeri</h2>
                            <article class="postbox post format-image mb-40 galleryVideos">
                                <div class="postbox__gallery mb-35">
                                    <?php foreach ($getGallery as $key => $value) { $bol = explode("v=",$value['link'])?>
                                        <img  class="" src="https://img.youtube.com/vi/<?php echo $bol[1]?>/hqdefault.jpg" data-url='<?php echo $value['link'];?>' alt="<?php echo $detectedServices['name']?> galeri videosu">
                                    <?php } ?>
                                </div>
                                <a class="clikGalleryItems popup-video video-btn setData" href=""><i class="fas fa-play"></i></a>
                            </article>
                        </div>
                    </div>
                </div>
