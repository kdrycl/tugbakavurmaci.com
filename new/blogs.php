...
if(isset($_GET['categorySlug'])){
    $getAllBlog = $dbclass->cek("ASSOC_ALL","blogsvscategories","blogs.*","INNER JOIN categories ON categories.id = blogsvscategories.categoryUniqueId INNER JOIN blogs ON blogs.uniqueId=blogsvscategories.blogUniqueId where categories.categorySlug=? ORDER BY blogs.id DESC",array($_GET['categorySlug']));
}
            ...
            <div class="container">
                ...
                <div class="col-lg-4">
                       ...
                       <div class="widget mb-40">
                           ...
                           <ul class="cat">
                           <?php $detectedBlogCategories = $dbclass->cek("ASSOC_ALL","categories","id,uniqueId,categoryName,categorySlug","where recordStatus=? ORDER BY rowNumber ASC LIMIT 0,6",array(1));
                            foreach ($detectedBlogCategories as $keyblogcat => $valueblogcat) { 
                                $countCat = $dbclass->cek("KAYITSAY","blogsvscategories","count(id)","where categoryUniqueId=?",array($valueblogcat['uniqueId']));
                                if($countCat>0){ ?>
                                    <li><a href="blog/kategori/<?php echo $valueblogcat['categorySlug'];?>"><?php echo $valueblogcat['categoryName'];?> <span class="f-right"><?php echo $countCat;?></span></a></li>   
                                <?php } ?>
                            <?php } ?>
                           </ul>
                       </div>
